...
const handlePost = async (req, res) => {
  ...
  try {
    ...
    const findNoticed = await CourseNotice.findOne({
      where: { course_id: id },
    });
    ...
    if (findNoticed) {
      ...
      return res.status(200).json({ addNotice: courseNoticed });
    } else if (!findNoticed) {
      ...
      return res.status(200).json({ addNotice });
    }
  } catch (e) {
    res.status(400).json({
